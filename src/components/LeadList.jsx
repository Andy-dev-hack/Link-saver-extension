...
export default function LeadList({ leads, folderName, isExpanded }) {
    ...
    return (
        <ul className={`list-none p-0 m-0 min-h-[10px] transition-all duration-300 ${isExpanded ? 'opacity-100 max-h-[1000px]' : 'opacity-0 max-h-0 overflow-hidden'}`}>
                ...
                <li key={lead.id || index} className="list-none">
                    <div className="flex justify-between items-center p-[10px] bg-dark-item mb-2 rounded-[5px] shadow-sm transition-all duration-200 hover:bg-dark-item-hover hover:translate-x-[5px] group/item">
                        <div className="flex items-center gap-2 grow overflow-hidden">
                            <span 
                                className="shrink-0 font-bold cursor-text px-1 rounded min-w-[50px] inline-block whitespace-nowrap overflow-hidden text-ellipsis max-w-[60%] focus:outline-none focus:shadow-[0_0_0_2px_#4a6ea9] focus:bg-[#383838] focus:whitespace-normal focus:overflow-auto text-text-main" 
                                contentEditable
